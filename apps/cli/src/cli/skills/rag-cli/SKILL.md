...
triggers: ["collections", "generate-dataset", "dataset", "config show", "version", "liste des commandes"]
---
...
| Command | What it does |
|---|---|
| `generate-dataset ./docs -o out.jsonl -n 20 --provider albert` | Generate synthetic Q&A evaluation dataset |
...
3. Recommend 20 questions for a first run (~2 minutes)
...
3. To enable a collection, read current list from `get_ragfacile_config()`
4. Compute new list and call `update_config("storage.collections", "[id1, id2, ...]")`
   with confirmation (never use run_rag_facile for this — use update_config)
