...
_DOCS_INDEX: dict[str, str] = {
    ...
    "ragtime.toml": "reference/ragfacile-toml.md",
...
def get_agents_md() -> str:
    ...
    if _workspace_root is None:
        return "No workspace detected. Run 'ragtime setup' to create a workspace."
    agents_file = _workspace_root / "AGENTS.md"
    if not agents_file.exists():
        return (
            "No AGENTS.md found in this workspace. "
            "This file is generated by 'ragtime setup' and describes the project "
...
def get_docs(topic: str) -> str:
    """Read ragtime documentation on a given topic.

    Use this for ragtime product questions: installation, setup workflow,
    evaluation, presets, CLI commands, or how a specific ragtime feature
    ...
    For current config values, prefer run_ragtime('config show') — it
    ...
    if not topic.strip():
        lines = ["Available ragtime documentation:\n"]
...
def activate_skill(name: str) -> str:
    ...
    - explain-rag      → user asks WHAT something IS or HOW it works, specifically about
                         RAG or ragtime concepts (chunking, embeddings, retrieval…).
    ...
    - explore-codebase → user asks WHERE something is implemented in the ragtime source,
...
# ── Memory tools ──────────────────────────────────────────────────────────────
# Re-exported from ragtime.memory.tool so they appear in this module's
# namespace alongside the other tools.  The agent harness registers them
# in the ToolCallingAgent tool list.

from ragtime.memory.tool import memory_edit  # noqa: E402, F401
from ragtime.memory.tool import memory_read  # noqa: E402, F401
from ragtime.memory.tool import memory_search  # noqa: E402, F401
from ragtime.memory.tool import memory_write  # noqa: E402, F401
from ragtime.memory.tool import set_workspace_root as _set_memory_workspace  # noqa: E402, F401
...
@tool
def run_ragtime(subcommand: str) -> str:
    """Run a ragtime CLI subcommand and return its output.
    ...
    # Use workspace as cwd when available (for commands that read ragtime.toml).
    ...
    try:
        ...
        return "ragtime CLI not found. Ensure the package is installed."
