...
class AlbertPipeline(RAGPipeline):
    ...
    def _get_chunks(
        ...
        query: str,
        **kwargs: object,
    ) -> tuple[list[Any], list[int | str], list[str], list[dict], list[dict], Any]:
        """Run expand → search → fuse → rerank; return results for both
        :meth:`process_query` and :meth:`retrieve_chunks`.

        Returns:
            ``(chunks, collection_ids, expanded_queries, pre_rerank, reranked, config)``
            ...
            - *config*: resolved RAGConfig (avoids a second ``get_config()`` call in callers)
        """
        ...
        if not chunks:
            return [], collection_ids, expanded_queries, [], [], config
        ...
        return chunks, collection_ids, expanded_queries, pre_rerank, reranked, config
    ...
    def retrieve_chunks(self, query: str, **kwargs: object) -> list[Any]:
        ...
        """
        chunks, _, _, _, _, _ = self._get_chunks(query, **kwargs)
        return chunks
