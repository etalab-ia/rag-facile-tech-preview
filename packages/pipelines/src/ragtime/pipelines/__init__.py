...
query expansion) is independently configurable via ``ragtime.toml``.
...
Convenience functions are also available for simpler call-sites::
    ...
    from ragtime.pipelines import process_file, process_bytes, get_accepted_mime_types
"""
...
def get_pipeline(config: Any | None = None) -> RAGPipeline:
    ...
    Reads ``ragtime.toml`` to instantiate and wire the phase providers.
...
#   from ragtime.pipelines import process_file
