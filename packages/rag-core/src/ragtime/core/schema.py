"""Pydantic models for Ragtime configuration.
...
class RAGConfig(BaseModel):
    ...
    This model represents the entire ragtime.toml configuration file,
    covering all phases of the RAG pipeline from document ingestion to
    response formatting.

    Example:
        >>> import tomllib
        >>> with open("ragtime.toml", "rb") as f:
...
class PipelineStage:
    ...
    Each stage maps to a section in ``ragtime.toml`` and carries display
...
def flatten_model_fields(
    ...
    Example:
        >>> from ragtime.core.schema import IngestionConfig, flatten_model_fields
