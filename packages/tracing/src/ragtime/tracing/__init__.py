...
Typical usage from the pipeline::
    ...
    from ragtime.tracing import get_tracer, set_current_trace_id
...
Typical usage from an application (after LLM generation)::
    ...
    from ragtime.tracing import get_tracer, get_current_trace_id
...
def _resolve_db_path(raw_path: str) -> Path:
    ...
    containing ``ragtime.toml`` (found by walking up from CWD).
    ...
    # Walk up from CWD to find ragtime.toml → its parent = workspace root
    from ragtime.core.loader import _find_config_file

    config_file = _find_config_file()
    if config_file is not None:
        return config_file.parent / db

    # Fallback: resolve relative to CWD (standalone projects without ragtime.toml)
    return Path.cwd() / db
...
def get_tracer(config: Any | None = None) -> TracingProvider:
    ...
    Reads ``[tracing]`` from ``ragtime.toml`` on first call.
    Subsequent calls return the cached instance.

    Args:
        config: Optional RAGConfig.  If *None*, loads from ragtime.toml.
                ...
                case "postgres":
                    ...
                    if not conn_str:
                        msg = (
                            ...
                            "Set tracing.connection_string in ragtime.toml "
