"""Custom Inspect AI solvers for ragtime evaluation."""
...
def _call_pipeline(question: str) -> tuple[str, list[str]]:
    ...
    Uses :meth:`~ragtime.pipelines.RAGPipeline.retrieve_chunks` to fetch
    chunks in a single Albert API call, then formats them locally via
    :func:`~ragtime.context.format_context`.  This avoids the double-search
...
def retrieve_rag_context() -> Solver:
    ...
    Calls :meth:`~ragtime.pipelines.RAGPipeline.retrieve_chunks` using the
    collections configured in ``ragtime.toml``.  The retrieved context is
