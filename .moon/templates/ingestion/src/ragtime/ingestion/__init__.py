...
from ragtime.ingestion._base import IngestionProvider


def get_provider(config: Any | None = None) -> IngestionProvider:
    """Get the configured ingestion provider.

    Reads ``ragtime.toml`` to determine which backend to use.

    Args:
        config: Optional RAGConfig instance. If None, loads from ragtime.toml.
    ...
    match backend:
        ...
        case "albert":
            raise ValueError(
                ...
                'will be removed in 0.5.0. Use provider = "local" in ragtime.toml, '
